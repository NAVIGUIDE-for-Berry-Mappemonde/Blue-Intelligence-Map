...
agent_communication:
    - agent: "main"
      message: "Applied surgical fix to /app/frontend/src/components/MapView.js only (scope-strict, App.js untouched). Frontend hard-restarted (killed react-scripts + supervisord restart) so the served bundle is guaranteed fresh. Please verify the 4 assertions above via headless Playwright on the preview URL https://formal-mode-refactor.preview.emergentagent.com. Do NOT rely on screenshots — measure DOM directly."
    - agent: "testing"

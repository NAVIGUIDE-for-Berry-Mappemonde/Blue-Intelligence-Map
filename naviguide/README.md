...
- **AI expedition briefing** generated via a multi-agent pipeline backed by an LLM cascade (NVIDIA NIM → OpenRouter → Claude)
...
All LLM steps go through the shared cascade adapter `llm_cascade.py`
(NVIDIA NIM → OpenRouter → Anthropic Claude — a provider without an API key
is skipped; with no key at all, structured fallback text is used):
...
POST /api/v1/expedition/plan/berry-mappemonde
         ...
         ▼
┌──────────────────────────────┐
│ Agent 1 — Route Intelligence │  LLM cascade (NIM → OpenRouter → Claude)
│ Anti-shipping score analysis │
│ Per-segment route quality    │
└──────────────┬───────────────┘
               │
               ▼
┌──────────────────────────────┐
│ Agent 3 — Risk Assessment    │  LLM cascade (NIM → OpenRouter → Claude)
...
└──────────────┬───────────────┘
               ...
               ▼
┌──────────────────────────────┐
│ Briefing — Executive Summary │  LLM cascade (NIM → OpenRouter → Claude)
│ Skipper-grade briefing       │
│ FR/EN, max 280 words         │
└──────────────────────────────┘
```
...
- At least one LLM cascade key — `NVIDIA_API_KEY` (NIM, preferred), `OPENROUTER_API_KEY`, or `ANTHROPIC_API_KEY` (last resort). Without any key, briefings/agents fall back to structured static text.
...
# LLM cascade: NVIDIA NIM → OpenRouter → Claude (any subset of keys works)
...
# 2. Set at least one LLM cascade key and start supervisord
...
- **LLM cascade** — briefing and agents 1 & 3 need at least one key among `NVIDIA_API_KEY` / `OPENROUTER_API_KEY` / `ANTHROPIC_API_KEY`; falls back to static analysis text if none is available

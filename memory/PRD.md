...
## Update 2026-08-26 (bis) — Rafraîchissement automatique + Génération mondiale
- **Auto-refresh sans action manuelle** (poe_routes.py): boucle asyncio démarrée au startup (start_auto_refresh). Tick 30 min, cycle toutes les 12 h (si aucun batch/référentiel manuel en cours) : zones ia/ia_sans_source re-vérifiées après 30 j (REFRESH_AFTER_DAYS), zones erreur re-tentées force après 7 j (ERROR_RETRY_DAYS), cap 60 zones/cycle, séquentiel. GET /api/poe/auto-refresh/status (config+summary+logs). UI: section ♻ dans la carte SIA formalités (poll 20 s) avec résumé du dernier cycle.
- **Monitoring MD5 prioritaire** (poe.py generate_zone_poe): avant TOUTE recherche, re-fetch des sources CONNUES (zone.source_hashes) — si hash du TEXTE extrait identiques → skip total (0 appel Gemini/LLM/géocodage), checked_at mis à jour. IMPORTANT: hash calculé sur le texte trafilatura/PyMuPDF, PAS sur les bytes bruts (HTML à tokens dynamiques → hash instable, bug corrigé et validé: cycle Fidji backdaté -31 j → 'MD5 inchangés — ré-extraction sautée' en 2 s, unchanged_md5=1).

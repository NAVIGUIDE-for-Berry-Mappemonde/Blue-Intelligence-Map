...
/**
 * Couche Marinas : dump mondial OSM, clusters Leaflet.
 * Les popups lisent tRef.current à l'ouverture (bascule FR ↔ EN).
 */
export default function useMarinasLayer({ mapObj, marinaClusterRef, marinaMarkersById, marinas, tRef }) {
  ...
  useEffect(() => {
    ...
    const sig = `${feats.length}:${first.osm_id || first.id || ""}:${last.osm_id || last.id || ""}`;

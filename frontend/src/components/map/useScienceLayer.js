...
/**
 * Couche Science : jeux de données océano (Sextant/ODATIS/EDMED) + flotteurs
 * Argo (dernier profil). Jeux de données = disque plein, Argo = anneau creux.
 * Popup : organisme, résumé, DOI, lien direct vers la fiche du portail.
 */
export default function useScienceLayer({
  mapObj, clusterRef, markersById, science, tRef,
}) {
  const sigRef = useRef("");
  ...
  useEffect(() => {
    ...
    const sig = `${feats.length}:${argoCount}:${first.id || ""}:${last.id || ""}`;
    ...
    const markers = feats.map((f) => {
      ...
      m.bindPopup(
        () => {
          ...
          const displayName = p.name || t("scienceUnnamed");
          const kindLabel = isArgo ? t("scienceArgoFloat") : t("scienceDataset");
        ...
        { maxWidth: 330, maxHeight: 420, autoPan: true, autoPanPadding: [40, 40] },
      );

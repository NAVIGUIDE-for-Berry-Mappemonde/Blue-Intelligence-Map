...
export default function MarinasCard({ t, showAnchorages, setShowAnchorages, anchoragesCount, rulesPayload }) {
  ...
  const startBuild = async () => {
    ...
    try { await api.post("/marinas/build", { resume: true, clear_before: false, ...extra() }); }
  ...
  const startAnchBuild = async () => {
    ...
    try { await api.post("/anchorages/build", { include_corridor: corridorOn, clear_before: false, ...extra() }); }
    catch (e) { alert(e.response?.data?.detail || e.message); }
  ...
  const startMarinaBatch = async () => {
    ...
    try { await api.post("/marinas/enrich-batch", { limit: lim, ...extra() }); }
    catch (e) { alert(e.response?.data?.detail || e.message); }
  ...
  const startMapsPlace = async () => {
    ...
    try { await api.post("/marinas/maps-place", { limit: 0, force: false, skip_search: true, ...extra() }); }
    catch (e) { alert(e.response?.data?.detail || e.message); }

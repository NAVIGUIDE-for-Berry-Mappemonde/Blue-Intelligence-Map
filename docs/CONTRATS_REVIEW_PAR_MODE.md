...
L’URL est-elle une **page projet** (pas une home fondation, donate, news, jobs) ?

Chaque `sites[]` est-il un **lieu d’action** assez précis et accessible en bateau — pas le siège, pas une ville intérieure, pas un `snap_to_ocean`, pas un `ocean_fallback` ?

S’il n’y a aucun site tenable : `unlocated` (reste en file), pas un GPS inventé.
...
Est-ce bien une marina de plaisance visitable (`leisure=marina`), au bon GPS, sans la confondre avec un PoE ni avec la capitainerie d’à côté ?
...
Le point est-il le **bâtiment** harbour-master, pas un ponton, pas un `leisure=marina` ?

Le téléphone et le VHF viennent-ils des tags ou d’une page officielle — pas d’un avis TripAdvisor, pas d’une invention LLM ?

Le calque SHOM/NOAA colle-t-il le **même** bâtiment (≤ 250 m) ou a-t-on fusionné deux bureaux ?

...
export default function App() {
  ...
  const officialClock = voyage.clock;
  ...
  useFilmCamera({
    mapRef,
    mapReady,
    enabled: routeReady && Boolean(cast?.follow || (isSuivre && live)),
  ...
  useCatamaranMarker(mapRef, {
    visible: routeReady && isSimulation && Boolean(cast?.main),
    lat: cast?.main?.lat,
    lon: cast?.main?.lon,
  ...
  useCatamaranMarker(mapRef, {
    visible: routeReady && isSuivre && Boolean(live) && cast?.vehicle !== "plane",
    lat: live?.lat,
    lon: live?.lon,
  ...
  useCatamaranMarker(mapRef, {
    visible: routeReady && isSuivre && previewing && Boolean(cast?.main),
    lat: cast?.main?.lat,
    lon: cast?.main?.lon,
    bearing: cast?.main?.bearing || 0,
    onDrag: undefined,
  ...
  useCatamaranMarker(mapRef, {
    visible: routeReady && Boolean(cast?.side?.visible),
    lat: cast?.side?.lat,
    lon: cast?.side?.lon,
  ...
  usePlaneMarker(mapRef, {
    visible: routeReady && Boolean(cast?.plane?.visible),
    lat: cast?.plane?.lat,
    lon: cast?.plane?.lon,

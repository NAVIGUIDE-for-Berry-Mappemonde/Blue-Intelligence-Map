...
- **Capitaineries — World dump**: tiled Overpass `office=harbour_master` / `seamark:building:function=harbour_master` worldwide, then SHOM WFS `buisgl_point` FUNCTN=2 plus SMCFAC CATSCF=6, then NOAA ENC Direct BUISGL FUNCTN=2. Phone and VHF from tags, TinyFish Search/Fetch + regex, NVIDIA NIM (Pro → Muse → gpt-oss), then OpenRouter. No marina linking.
